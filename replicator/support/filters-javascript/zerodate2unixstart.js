 ...
 *
 * This filter turns MySQL 0 date and timestamp values into 1970-01-01 values.
 * It is necessary for MySQL to Oracle replication, since Oracle gets sick
 * on 0 date values that MySQL accepts. 
 *
 * @author <a href="mailto:robert.hodges@continuent.com">Robert Hodges</a>
 * @author <a href="mailto:sascha.dienelt@intelliad.de">Sascha Dienelt</a>
 */
		...
		for (c = 0; c < columns.size(); c++) {
			...
			if (type == TypesDATE || type == TypesTIMESTAMP) {
				...
				for (row = 0; row < columnValues.size(); row++) {
					...
					if (value.getValue() == 0) {
						if (type == TypesDATE) {
							value.setValue(new java.sql.Date(0));
							logger.debug("Fixed up a date by making it 1970-01-01");
						} else {
							value.setValue(new java.sql.Timestamp(0));
							logger.debug("Fixed up a datetime by making it 1970-01-01 00:00:00");
						}
					} else {
